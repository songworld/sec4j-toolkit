...
import com.toolkit2.client.frame.free.FreeToolbarToggleButton;
...
public class TreeToolbar extends JPanel
{
  ...
  private FreeToolbarButton btnUp = new FreeToolbarButton(null, FreeIconStore.UP_ICON, GeneralUtil.getString("TreeToolbarFactory.Moveup"));
  private FreeToolbarButton btnDown = new FreeToolbarButton(null, FreeIconStore.DOWN_ICON, GeneralUtil.getString("TreeToolbarFactory.Movedown"));
  private FreeToolbarButton btnTop = new FreeToolbarButton(null, FreeIconStore.TOP_ICON, GeneralUtil.getString("TreeToolbarFactory.Movetotop"));
  private FreeToolbarButton btnBottom = new FreeToolbarButton(null, FreeIconStore.BOTTOM_ICON, GeneralUtil.getString("TreeToolbarFactory.Movetobottom"));
  private FreeToolbarToggleButton btnAscendingSort = new FreeToolbarToggleButton(null, FreeIconStore.ASCENDING_SORT_ICON, GeneralUtil.getString("TreeToolbarFactory.Sortinascendingorder"));
  private FreeToolbarToggleButton btnDescendingSort = new FreeToolbarToggleButton(null, FreeIconStore.DESCENDING_SORT_ICON, GeneralUtil.getString("TreeToolbarFactory.Sortindescendingorder"));
  private FreeToolbarToggleButton btnDefault = new FreeToolbarToggleButton(null, FreeIconStore.DEFAULT_ICON, GeneralUtil.getString("TreeToolbarFactory.Sortindefaultorder"));
  private FreeToolbarToggleButton btnCustomizedSort = new FreeToolbarToggleButton(null, FreeIconStore.CUSTOM_ORDER_ICON, GeneralUtil.getString("TreeToolbarFactory.Sortincustomizedorder"));
  private FreeToolbarToggleButton btnMy2BizBox = new FreeToolbarToggleButton(null, FreeIconStore.CONFIG_ICON, GeneralUtil.getString("SystemMenu.Config2BizBoxFree"));
  ...
  public TreeToolbar(final TTree tree) {
    ...
    this.btnUp.addActionListener(new ActionListener()
    {
      public void actionPerformed(ActionEvent e)
      {
        ...
        Iterator it = ((Element)tree.getDataBox().getRootElements().get(0)).children();
        while (it.hasNext()) {
    ...
    this.btnDown.addActionListener(new ActionListener()
    {
      public void actionPerformed(ActionEvent e) {
        ...
        boolean startToIterate = false;
        List list = ((Element)tree.getDataBox().getRootElements().get(0)).getChildren();
    ...
    {
      public void actionPerformed(ActionEvent e) {
        TreeToolbar.sortTreeNode(tree.getDataBox(), true);
        TreeToolbar.this.resetMoveButtons();
        ClientUtil.saveClientProperty(TreeToolbar.TREE_SORT_ORDER, TreeToolbar.ASCENDING_SORT);
      }
    });
    this.btnDescendingSort.addActionListener(new ActionListener()
    {
      public void actionPerformed(ActionEvent e) {
        TreeToolbar.sortTreeNode(tree.getDataBox(), false);
        TreeToolbar.this.resetMoveButtons();
        ClientUtil.saveClientProperty(TreeToolbar.TREE_SORT_ORDER, TreeToolbar.DESCENDING_SORT);
      }
    });
    this.btnDefault.addActionListener(new ActionListener()
    {
      public void actionPerformed(ActionEvent e) {
        ...
        while (it.hasNext()) {
          Element element = (Element)it.next();
          elements.put(element.getClientProperty("defaultIndex"), element);
        }
        ...
        TreeToolbar.this.resetMoveButtons();
        ClientUtil.saveClientProperty(TreeToolbar.TREE_SORT_ORDER, TreeToolbar.DEFAULT_SORT);
      }
    });
    this.btnCustomizedSort.addActionListener(new ActionListener()
    {
      public void actionPerformed(ActionEvent e) {
        TreeToolbar.this.resetMoveButtons();
        ClientUtil.saveClientProperty(TreeToolbar.TREE_SORT_ORDER, TreeToolbar.CUSTOMIZED_SORT);
      }
    });
   
    tree.getDataBox().getSelectionModel().addDataBoxSelectionListener(new DataBoxSelectionListener()
    {
  ...
  private void resetMoveButtons()
  {
    ...
    if (!this.tree.getDataBox().getSelectionModel().isEmpty()) {
      ...
      Iterator it = this.tree.getDataBox().getSelectionModel().selection();
      while (it.hasNext()) {
        Element element = (Element)it.next();

        if (((element.getParent() != null) && (element.getParent().getParent() != null)) || (element.getParent() == null)) {
          allSelectionAreBoxModules = false;
          break;
        }
        allSelectionAreBoxModules = true;
      }
      buttonEnableStatus = (buttonEnableStatus) && (allSelectionAreBoxModules);
    }
  ...
  public static void sortTreeNode(TDataBox box, boolean ascendingSort) {
    ...
    while (it.hasNext()) {
      ...
      if (((index instanceof Integer)) && (((Integer)index).intValue() > lastNodeIndex.intValue())) {
